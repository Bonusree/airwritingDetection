"""Small NumPy modeling utilities for procedure2 feature baselines.

Identical to `procedure1/feature_modeling/common/ml_utils.py`; kept as a separate
copy so each procedure stays self-contained and independently reproducible.
"""
...
def run_nested_cv(
    *,
    ...
    for outer_fold_id in outer_fold_ids:
        ...
        for config_index, config in enumerate(configs):
            ...
            for inner_fold_id in inner_fold_ids:
                ...
                if not train_ids or not val_ids:
                    # A modality (e.g. trajectory images) can be entirely
                    # missing for every writer in this inner train/val split.
                    continue
        ...
        if not test_ids:
            # Some procedure2 modalities (e.g. trajectory images) are missing
            # for every sample belonging to this fold's held-out writer, so
            # there is nothing to evaluate here; record it rather than crash.
            outer_metric_rows.append(
                {
                    "baseline": baseline_name,
                    "outer_fold_id": outer_fold_id,
                    "selected_config": selected["config_name"],
                    "train_samples": len(development_ids),
                    "test_samples": 0,
                    "missing_test_labels_from_train": "",
                    "accuracy": 0.0,
                    "balanced_accuracy": 0.0,
                    "macro_f1": 0.0,
                    "top2_accuracy": 0.0,
                    "note": "no_test_samples_for_this_modality",
                }
            )
            continue
...
def aggregate_metrics(baseline_name: str, outer_metric_rows: list[dict[str, Any]], labels: list[str]) -> dict[str, Any]:
    ...
    # A fold with zero test samples (a modality missing for an entire held-out
    # writer, e.g. images for user5) isn't a real evaluation; a 0.0 for it
    # would only drag the mean down, so it is excluded from the average but
    # still shown in outer_fold_metrics.csv and counted below.
    scored_rows = [row for row in outer_metric_rows if int(row.get("test_samples", 0)) > 0]
    skipped_folds = [int(row["outer_fold_id"]) for row in outer_metric_rows if int(row.get("test_samples", 0)) == 0]
    aggregate: dict[str, Any] = {
        "baseline": baseline_name,
        "outer_folds": len(outer_metric_rows),
        "outer_folds_scored": len(scored_rows),
        "outer_folds_skipped_no_test_samples": skipped_folds,
        "labels": labels,
        "metrics": {},
    }
    for metric_name in metric_names:
        values = np.array([float(row[metric_name]) for row in scored_rows], dtype=np.float64)
